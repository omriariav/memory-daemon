...
def _memory_body(task, tasklist_title):
...
def _generated_metadata_due(metadata):
    """Return ``(is_generated_metadata, due)`` for our exact body trailer."""
    lines = metadata.splitlines()
    if not lines or len(lines) > 2 or not lines[0].startswith("List: "):
        return False, None
    if len(lines) == 1:
        return True, None
    if not lines[1].startswith("Due: "):
        return False, None
    due = lines[1][5:].strip()
    return (True, due) if DATE_RE.fullmatch(due) else (False, None)
...
def _memory_to_google(entry):
    ...
    if managed and f"\n\n{MEMORY_MARKER}\n" in body:
        ...
        generated, candidate_due = _generated_metadata_due(metadata)
        if generated:
...
def _memory_upsert(store, task, tasklist, source_id, update_id=None):
    body = _memory_body(task, tasklist["title"])
    tags = f"google-tasks,google-tasks-{_slug(tasklist['title'])}"
    args = [
        "add", "--title", str(task.get("title") or "Untitled task"),
        "--type", "todo", "--date", _today(),
...
def run(cfg, checkpoint_path=None, dry_run=False):
    ...
    for key, task in sorted(google_tasks.items()):
        ...
        if not mapping and not entry:
            ...
            if len(exact) == 1:
                ...
                if not dry_run:
                    _assert_pair_unchanged(store, entry, task)
                    _memory_upsert(store, task, tasklists[list_id], source_id, entry["id"])
                    entry = dict(
                        entry,
                        title=str(task.get("title") or "Untitled task"),
                        body=_memory_body(task, tasklists[list_id]["title"]),
    ...
    for entry in sorted(memory_entries.values(), key=lambda row: (row["date"], row["id"])):
        ...
        _memory_upsert(store, task, tasklists[outbound_list], source_id, entry["id"])
        _commit_store(str(store), "memory: sync Google Tasks")
        synced_entry = dict(
            entry,
            title=str(task.get("title") or entry["title"]),
            body=_memory_body(task, tasklists[outbound_list]["title"]),
            source_ids=[*entry["source_ids"], source_id],
        )
        mappings[key] = {
            "memory_id": entry["id"],
